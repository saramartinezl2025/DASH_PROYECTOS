...
function buildChartModel(rows) {
  ...
  /** Categorías que no deben aparecer como pétalos del rosa. */
  ...
  const rowSum = (row) =>
    orderedTypes.reduce((s, tipo) => s + (Number(row[tipo]) || 0), 0);
  chartData.sort(
...
export default function PolarEscuelasChart({ soloVencidas = false }) {
  ...
  const { orderedTypes, schools, totalsByType, palette } = useMemo(
    () => buildChartModel(rows),
    [rows],
  );
  ...
  const header = (
    <header className="nightingale-header">
      ...
      <p className="nightingale-header__subtitle">
        Rosa de Nightingale · Área proporcional al volumen · Fuente: public.factory_requests
